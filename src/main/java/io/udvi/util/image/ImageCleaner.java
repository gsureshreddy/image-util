...
public class ImageCleaner {
    ...
    public static void main (String args[]) throws IOException {
        List<Map<String, Integer>> paths = findDuplicateImagePairs("/Volumes/SURESHREDDY/PERSONAL_FILES/");
        long i = 0;
        for ( Map<String, Integer> pathmap : paths) {
            if (pathmap.size() == 1) {
                String url = pathmap.keySet().iterator().next();
                Files.move(Paths.get(url), Paths.get("/Volumes/SURESHREDDY/PHOTOS_UNIQUE/PHOTO_" + i + url.substring(url.lastIndexOf("."))), StandardCopyOption.REPLACE_EXISTING);
                i++;
            }


        }
        //List<String> uniqueImagePaths =  findAllUniqueImages("/Volumes/SURESHREDDY/test/");
        //System.out.println("Unique : " + findDuplicateImagePairs("/Volumes/SURESHREDDY/test/"));
    }
    ...
    private static List<Map<String, Integer>> findDuplicateImagePairs(String directoryPath){
        ...
        for(File imagesFile:imagesFileList){
            System.out.println(imagesFile.getName() + " -- " + (i+1) + "/" + imagesFileList.size());
            i++;
    ...
    private static List<File> getAllImagesFromDirectory(String directoryPath){
        ...
        if(file.isDirectory())
            imagesPathList = Arrays.asList(file.listFiles());
